...
class SendMixin:
    ...
    async def get_media_group(cls, client: Client, message: Message, text: str):
        ...
        logger.debug(f"[MSG ID {message.id}]: Media group содержит {len(medias)} файла.")
        ...
        if self.settings.include_media and message.media:
            ...
            if message.media_group_id:
                try:
                    ...
                    logger.warning(f"send_message: Exception while getting media group: {e}")
                ...
                logger.debug(f"[MSG ID {message.id}]: Отправляю медиа групп пост")
                return await bot.send_message(
                    self.get_receiver_chat_id(),
                    text,
                    reply_markup=markup,
            ...
            logger.debug(f"[MSG ID {message.id}]: Отправляю обычный медиа пост")
            return await self.send_media(client, bot, message, text, markup)

        else:  # message.text:
            logger.debug(f"[MSG ID {message.id}] [Media type: {message.media}]: Отправляю текстовый пост.")
            return await bot.send_message(
                self.get_receiver_chat_id(),
                text,
                reply_markup=markup,
