...
class Project(Base, CheckMixin, SendMixin):
    ...
    async def trigger(
        ...
        logger.debug(f"[Project {self.name}: MSG ID {message.id}]: ***СТАРТ ТРИГГЕРА*** ")

        try:
            if self.forward_all_messages:
                logger.debug(f"[MSG ID {message.id}]: Активирована пересылка всех постов")

                if message.media_group_id:
                    logger.debug(f"[MSG ID {message.id}]: Юзербот: {self.account_id} | Mediagroup ID: {message.media_group_id}")
                    if not hasattr(self, 'processed_media_groups'):
                        self.processed_media_groups = {}

                    if self.account_id not in self.processed_media_groups:
                        self.processed_media_groups[self.account_id] = []

                    if message.media_group_id not in self.processed_media_groups[self.account_id]:
                        self.processed_media_groups[self.account_id].append(message.media_group_id)
                        logger.debug(f"[MSG ID {message.id}]: Добавили медиагруппу в список {self.processed_media_groups}")
                        if not self.check_stop_conditions(message):
                            return
            ...
            logger.debug(f"[Message ID {message.id}]: Активирована пересылка по ключам")
            await self.process_message(client, bot, message, keyword, forward_all=False)
        except Exception as e:
            logger.exception(f"Error in trigger: {e}")
    ...
    async def process_message(
        ...
        logger.debug(f"[MSG ID {message.id}]: Начало обработки параметров")
        ...
        try:
            ...
            text = settings.prepare_action_keywords(message, self.keywords, prepare_text) #замена текста и добавление подписи
            markup = settings.prepare_response_buttons(message) #клавиатуры
            ...
            if message.poll:
                poll_kwargs = {
                    ...
                    "type": message.poll.type.value,  # Преобразование типа опроса в строку
                ...
                # Добавляем атрибуты только если они существуют
                if hasattr(message.poll, 'explanation_parse_mode'):
                    poll_kwargs["explanation_parse_mode"] = message.poll.explanation_parse_mode
                if hasattr(message.poll, 'open_period'):
                    poll_kwargs["open_period"] = message.poll.open_period
                if hasattr(message.poll, 'close_date'):
                    poll_kwargs["close_date"] = message.poll.close_date

                await bot.send_poll(**poll_kwargs)
                logger.debug(f"[MSG ID {message.id}] [Media type: POLL]: Сообщение отправлено.")
            else:
                sm = await self.send_message(client, bot, message, text, markup)
                await asyncio.sleep(1)

                if forward_all and message.media_group_id:
                    await asyncio.sleep(1)
                    if message.media_group_id in self.processed_media_groups[self.account_id]:
                        self.processed_media_groups[self.account_id].remove(message.media_group_id)
                    logger.debug(f"[MSG ID {message.id}]: Список медиагрупп после очистки {self.processed_media_groups}")
                logger.debug(f"[MSG ID {message.id}] [Media type: {message.media}]: Сообщение отправлено.")
            ...
            if 'flood' in str(e).lower():
                logger.warning(f"Чат {message.chat.id} получил FLOOD ограничение на {e.retry_after} секунд. Добавляем в игнор-лист.")
            ...
            ignore_errors = ('text buttons are unallowed', 'must be non-empty')
    ...
    async def remove(self, session: AsyncSession, bot: Bot, account_dispatchers):
        ...
        if self.sender_id and account_dispatchers:  # Если у проекта есть отправитель
            ...
            logger.info(f"[REMOVE-PROJECT] Сохранённый sender chat ID: {self.sender_id}, Реальный sender chat ID: {project_sender_id}")
            has_similar = False
            ...
            for suspect_project in all_project_like_main_sender_chat_id:
                ...
                if suspect_project.id == self.id:  # Пропускаем проверку на текущем проекте
                    continue
                # Если основной чат проверяемого получателя такой же как и у основного получателя проекта, то не выходить из данной группы
                if suspect_project_sender_id == project_sender_id:
                    has_similar = True
                    break

            if not has_similar:  # Если нет проектов с таким же чатом получателем - то выйти из группы
                try:
                    ...
                    logger.error(f"[REMOVE-PROJECT] Ошибка при выходе из чата отправителя с ID: {project_sender_id}: {str(e)}")

        if self.receiver_id:  # Если у проекта есть получатель
            project_receiver_id = self.get_receiver_chat_id()
            logger.info(f"[REMOVE-PROJECT] Сохранённый receiver chat ID: {self.receiver_id}, Реальный receiver chat ID: {project_receiver_id}")
            has_similar = False
            # Все проекты в id которого есть данный чат
            all_project_like_main_receiver_chat_id = await Project.filter(session,Project.receiver_id.cast(String).like(f"{project_receiver_id}%"))
            ...
            for suspect_project in all_project_like_main_receiver_chat_id:
                ...
                logger.info(f"[FILTER-RECEIVER-PROJECTS] Проверяем проект с ID: {suspect_project.id} для чата получателя с ID: {project_receiver_id}")
            ...
            if not has_similar:  # Если нет проектов с таким же чатом получателем - то выйти из группы
                try:
                    ...
                    logger.error(f"[REMOVE-PROJECT] Ошибка при выходе из чата получателя с ID: {project_receiver_id}: {str(e)}")
    ...
    async def remove_project_by_project_id(
        ...
        try:
            ...
            if project.sender_id and account_dispatchers:
                ...
                logger.info(f"[REMOVE-PROJECT] Сохранённый sender chat ID: {project.sender_id}, Реальный sender chat ID: {project_sender_id}")

                has_similar_sender = False
                all_project_like_main_sender_chat_id = await Project.filter(session, Project.account_id == project.account_id, Project.sender_id.cast(String).like(f"{project_sender_id}%"))
