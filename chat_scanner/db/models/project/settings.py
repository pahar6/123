...
class ProjectSettings(Base):
    ...
    def re_text_v4(self, key, text, new_text):
        ...
        for i, row in enumerate(key):
            if i < n - 1:
                p += r'(<\w+[^\>]*>){0,}(</\w+>){0,}' + re.escape(row) + r'(<\w+[^\>]*>){0,}(</\w+>){0,}'
            else:
                p += re.escape(row)
        ...
        for i in range(len(lines)):
            ...
            logger.debug(f"Processing line {i}: {original_line}")

            # Перебираем найденные паттерны в обратном порядке
            for pattern in list(re.finditer(p, line, re.IGNORECASE))[::-1]:
                html = re.search(r'(<.*>)', pattern.group(0))
                start, stop = pattern.span()
                logger.debug(f"Found pattern: {pattern.group(0)} at {start}-{stop}")
                ...
                if new_text is not None:
                    ...
                    logger.debug(f"Line after replacement: {line}")
                else:
                    if html:
                        x_text = re.sub(fr'{re.escape(pattern.group(0))}(.*?</{html.group(0)[1]}>)?', '', line[start:])
                        line = line[:start] + x_text
                    else:
                        line_new = ''
                        line = line[:start] + line_new + line[stop:]
                    logger.debug(f"Line after removal: {line}")

            # Проверка, если строка стала пустой или содержит только пробелы, удаляем ее
            if new_text is None and original_line.strip() != '' and line.strip() == '':
                logger.debug(f"Line {i} is empty after processing and will be removed.")
                lines[i] = None
            else:
                lines[i] = line
    ...
    def prepare_response_text(
        ...
        logger.debug(f"[MSG ID {message.id}]: Текст после обработки параметров: {response}")
        return response
    ...
    def prepare_action_keywords(
        ...
        if prepare_text is None:
            logger.debug("prepare_text is None, initializing to an empty string.")
            prepare_text = ""

        text = message.text or message.caption or ""

        # Сначала удалим все возможные старые подписи
        # logger.info(
        #     f"Ключевые слова перед удалением подписей: {[kw.keyword for kw in keywords if kw.type.lower() == 'signature']}")
        for keyword in keywords:
            if keyword.type.lower() == 'signature' and keyword.keyword.strip():
                # logger.info(f"УДАЛЯЕМ ПОДПИСЬ: '{keyword.keyword}'")
                prepare_text = prepare_text.replace(f'\n\n{keyword.keyword}', '')

        for keyword in keywords[::-1]:
            if keyword.type.lower() == 'replacing':
                # Проверка на наличие ключевого слова в тексте
                if keyword.keyword.lower() in text.lower():
                    logger.debug(f"Текст на вход '{prepare_text}'")
                    logger.debug(f"Слово подлежащее замене '{keyword.keyword}' найдено в тексте.")
                    logger.debug(f"Слово для замены в тексте: '{keyword.replacing_text}'")
                    prepare_text = self.re_text_v4(key=keyword.keyword, text=prepare_text, new_text=keyword.replacing_text)
                else:
                    logger.debug(f"Слово подлежащее замене '{keyword.keyword}' не найдено в тексте.")
                    continue  # Пропускаем текущий keyword и переходим к следующему
            elif keyword.type.lower() == 'signature' and keyword.keyword.strip():
                logger.debug(f"ДОБАВЛЯЕМ ПОДПИСЬ: '{keyword.keyword}'")
                prepare_text += f'\n\n{keyword.keyword}'
                # elif keyword.type.lower() == 'signature':
            #     if keyword.keyword != '': #вот я про это место тебе говорю
            #         prepare_text += f'\n\n{keyword.keyword}'
        logger.debug(f"[MSG ID {message.id}]: Текст после автозамены и подписи {prepare_text}")
        return prepare_text
    ...
    def check(self, message: Message) -> bool:
        ...
        logger.debug(f"[MSG ID {message.id}]: ПРОВЕРКА ДУБЛИКТА текст/отправитель")
