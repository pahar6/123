...
async def mailings(bot: Bot, text: str, users: list | int, *, reply_markup: ReplyMarkup = None, photo: str | None = None) -> list[types.Message]:
    ...
    for user in users:
        try:
            if photo:
                mails.append(await bot.send_photo(chat_id=user, photo=photo, caption=text, reply_markup=reply_markup))
            else:
                mails.append(await bot.send_message(chat_id=user, text=text, reply_markup=reply_markup))
